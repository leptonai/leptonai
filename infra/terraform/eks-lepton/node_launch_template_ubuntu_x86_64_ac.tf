...
resource "aws_launch_template" "ubuntu_x86_64_ac" {
  name_prefix = "${var.cluster_name}-ubuntu-x86_64-ac-"
  image_id    = local.regional_ubuntu_amis.x86_64_gpu
  ...
  user_data = base64encode(<<-SCRIPT
    ...
    # only run if the underlying instance is Ubuntu custom AMI
    # DO NOT USE "null" here, it will err with "part.0.content" is required, but no definition was found
    # ref. https://github.com/awslabs/amazon-eks-ami/blob/master/files/bootstrap.sh
    if ! bash /etc/check-nvidia-smi.sh; then
        # nvidia-smi check failed...
        # no need to proceed bootstrap
        # since GPU won't work
        # log here for debuggability
        echo FAIL check-nvidia-smi.sh
        exit 1
    fi
