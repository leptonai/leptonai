...
def run_terraform_apply(dir: str, name: str):
    ...
    git_clone = subprocess.Popen(
        ["git", "clone", "git@github.com:leptonai/infra-ops.git"],
        cwd=dir,
        stdin=sys.stdin,
        text=True,
        stdout=sys.stdout,
    ...
    console.print(
        f"Initizaling the installer (Terraform) in {dir}... Taking a few minutes..."
    )
    terraform_init = subprocess.Popen(
        ["terraform", "init"], cwd=t_dir, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    ...
    terraform_apply = subprocess.Popen(
        ["terraform", "apply", "-auto-approve", f"-var=cluster_name={name}"],
        cwd=t_dir,
        stdin=sys.stdin,
        text=True,
        stdout=sys.stdout,
    ...
    terraform_output = subprocess.Popen(
...
def run_terraform_destroy(dir: str, name: str):
    ...
    terraform_destroy = subprocess.Popen(
        ["terraform", "destroy", "-auto-approve", f"-var=cluster_name={name}"],
        cwd=t_dir,
        stdin=sys.stdin,
        text=True,
        stdout=sys.stdout,
