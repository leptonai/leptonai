...
- Slurm is exposed as nested commands with flag-based selectors (no positional resource arguments). Start with `lep slurm cluster list` and `lep slurm job list`; use `lep slurm cluster get --name <name>` (or `--id <namespace/name>`), `lep slurm job get --id <job-id>` (or `--name <job-name>`), `lep slurm job attempts ...`, and `lep slurm job logs ...` for detail. Add `--output json` where advertised when structured output is needed.
- Slurm job submission and cancellation still happen through native Slurm tools (`sbatch`, `squeue`, `scancel`) after connecting to the cluster. The CLI's Slurm job API is intentionally read-only. `lep slurm cluster ssh --name <name>` opens an interactive SSH session to a cluster login node (pass `--user` when the login name differs; `--print-only` shows the command instead); treat it as high-impact under the confirmation rules above.
- A Slurm Dev Pod belongs to the current user and can be listed, inspected, created, removed, or connected to under `lep slurm devpod`. `create` takes `--cluster <name-or-namespace/name>`; `get`, `remove`, and `ssh` select the pod with exactly one of `--name`, `--id`, or `--cluster`. Treat `create`, `remove`, and interactive `ssh` as high-impact operations under the confirmation rules above.
