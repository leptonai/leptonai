...
class WorkspaceError(Exception):
    ...
    def __str__(self):
        details = ', '.join(
            f"{key.replace('_', ' ').title()}: {value}" for key, value in vars(self).items() if value and key != 'args')
...
class WorkspaceUnauthorizedError(WorkspaceError):
    def __init__(self, workspace_id=None, workspace_url=None, auth_token=None):
        super().__init__("Unauthorized access to the workspace", workspace_id, workspace_url, auth_token)


class WorkspaceNotFoundError(WorkspaceError):
    def __init__(self, workspace_id=None, workspace_url=None, auth_token=None):
        super().__init__(
            "Workspace not found. If the workspace was just created, please wait for 10 minutes. Contact us if the "
            "workspace remains unavailable after 10 minutes."
            , workspace_id, workspace_url, auth_token)
