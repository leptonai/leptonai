...
def photon():
    """
    Manages photons locally and on the DGX Lepton.
...
def remove(name, local, id_, all_, public_photon):
    ...
    logged in to the DGX Lepton via `lep login`. If one has logged in, this
...
def list_command(local, pattern, public_photon):
    """
    Lists all photons. If one has logged in to the DGX Lepton via `lep login`,
    this command will list all photons in the DGX Lepton. Otherwise, or if
...
def run(
    ...
    """
    Runs a photon. If one has logged in to the DGX Lepton via `lep login`,
