...
pub fn command() -> Command {
    Command::new(NAME)
        .about("Resumes the dev-machine with ASG")
        .arg(
            Arg::new("LOG_LEVEL")
                .long("log-level")
                .short('l')
...
pub async fn execute(log_level: &str, spec_file_path: &str, skip_prompt: bool) -> io::Result<()> {
    ...
    if !skip_prompt {
        let options = &[
            "No, I am not ready to resume the dev-machine!",
            "Yes, let's resume the dev-machine!",
        ];
        let selected = Select::with_theme(&ColorfulTheme::default())
            .with_prompt("Select your 'resume' option")
            .items(&options[..])
            .default(0)
            .interact()
            .unwrap();
    ...
    log::info!("resuming dev-machine with ASG...");
    ...
    log::info!("setting ASG logical id '{asg_logical_id}' desired capacity to 1");
    autoscaling_manager
        .cli
